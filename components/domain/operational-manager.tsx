...
function blank(slug:ModuleSlug):Row {
  ...
  if(slug==="money") return {direction:"income",amount:"0",currency:"GBP",category:"Other",description:"",occurred_on:new Date().toISOString().slice(0,10)};
  return {number:"",client_id:"",amount:"0",currency:"GBP",status:"draft",issue_date:new Date().toISOString().slice(0,10),due_date:"",notes:""};
}

export function OperationalManager({slug,workspaceId,initialRows,clients,projects=[]}:{slug:ModuleSlug;workspaceId:string;initialRows:Row[];clients:ClientOption[];projects?:ProjectOption[]}) {
  const c=config[slug];
  const [rows,setRows]=useState(initialRows);
  const [query,setQuery]=useState("");
  const [open,setOpen]=useState(false);
  const [editing,setEditing]=useState<Row|null>(null);
  const [form,setForm]=useState<Row>(()=>blank(slug));
  ...
  function startCreate(){setEditing(null);setForm(blank(slug));setError(null);setOpen(true)}
  ...
  return <div className="page-enter space-y-7">
    ...
    <section className="grid gap-4 sm:grid-cols-3">
      ...
      {slug==="money"?<><Card className="p-5"><p className="text-xs text-muted-foreground">Income</p><p className="mt-3 text-3xl font-semibold">£{moneyIncome.toLocaleString()}</p></Card><Card className="p-5"><p className="text-xs text-muted-foreground">Net</p><p className="mt-3 text-3xl font-semibold">£{(moneyIncome-moneyExpense).toLocaleString()}</p></Card></>:<>
    ...
    <Card className="overflow-hidden">
      ...
      <div>{filtered.map(row=><div key={row.id} className="flex flex-col gap-3 border-b px-5 py-4 last:border-b-0 sm:flex-row sm:items-center">
        ...
        {row.amount!==undefined&&<p className="text-xs font-semibold">{row.currency||"GBP"} {Number(row.amount).toLocaleString()}</p>}
    ...
    {open&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" onMouseDown={e=>{if(e.currentTarget===e.target&&!pending)setOpen(false)}}>
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl border bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={save} className="mt-6 space-y-4"><Fields slug={slug} form={form} setForm={setForm} clients={clients} projects={projects}/>
...
function Fields({slug,form,setForm,clients,projects}:{slug:ModuleSlug;form:Row;setForm:(v:Row)=>void;clients:ClientOption[];projects:ProjectOption[]}){
